...
# kmeans evaluation utils from
# https://github.com/wvangansbeke/Unsupervised-Semantic-Segmentation/blob/main/segmentation/utils/kmeans_utils.py
# Authors: Wouter Van Gansbeke & Simon Vandenhende
# Licensed under the CC BY-NC 4.0 license (https://creativecommons.org/licenses/by-nc/4.0/)
# For convinience tqdm printing was added.


def _majority_vote(flat_preds, flat_targets, preds_k, targets_k):
    with tqdm_joblib(tqdm(desc="Majority vote", total=targets_k * preds_k)) as _:
        iou_mat = Parallel(n_jobs=N_JOBS, backend="multiprocessing")(
            delayed(get_iou)(flat_preds, flat_targets, c1, c2)
    ...
    return np.array(list(zip(range(preds_k), results)))


def _hungarian_match(flat_preds, flat_targets, preds_k, targets_k):
    assert preds_k == targets_k, f"{preds_k} is not equal to {targets_k}"  # one to one
    num_k = preds_k

    with tqdm_joblib(tqdm(desc="Hungarian matching", total=targets_k * preds_k)) as _:
